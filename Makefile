# Project variables
PROJECT_NAME ?= todobackend-play
ORG_NAME ?= jkuoctanner
REPO_NAME ?= todobackend-play
...
test:
	...
	@ docker cp $$(docker-compose -p $(DEV_PROJECT) -f $(DEV_COMPOSE_FILE) ps -q test):/application/target/test-reports/. reports
...
build:
	...
	@ docker cp $$(docker-compose -p $(DEV_PROJECT) -f $(DEV_COMPOSE_FILE) ps -q builder):/application/target/universal/stage/. stage
	${INFO} "Build complete"
